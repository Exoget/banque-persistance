# banque-persistance:
Ce projet s'inscrit dans le cadre de la montée en compétence sur Maven.
La version utilisée dans ce projet est la 3.6
### LifeCycle

La construction d'un projet Maven suit un cycle de vie bien spécifique, il y a trois cycles maven:
...
### Phase:
a titre d'exemple le lifecycle Default possede 23 phases puisqu'il est considèré comme le principal lifecycle de contruction, 
...
le cycle maven est retourné avec la configuration adéquate. Les plugins à utiliser leurs noms, leurs versions ainsi dans quelle phase ils
sont rattachés. Tout cela est défini dans le fichier de configuration maven ``components.xml`` sous le repertoire ``MAVEN_HOME/lib/maven-core-3.0.2.jar!META-INF/plexus/components.xml``.

Voici le cycle DEFAULT pour un projet de type jar : 
...
Lors de l'exécution d'un cycle maven, maven va lancer tous les goals qui sont définie par défaut pour chaque phase.
...
* runtime ( n'est pas nécessaire à la compilation, mais à l'exécution )
* provided ( au moment de l'exécution elle sera fourni soit par la JDK, soit par le Server d'application )
* test (disponible seulement pour les tests unitaires)
...
Et pour l'utilisation, c'est fait de la manière suivante :
...
Une autre approche utilisée dans beaucoup de projets, c'est l'utilisation d'un module local contenant seulement les fichiers de conf et impoter avec le scope test parmi les dépendances projet.

#### Tests parameteres
* ```-Dmaven.test.skip=true``` : annuler l'exécution des tests unitaires.
...
## Référentiels Maven
Il y a 3 types :
* ```<repository>``` : element pour déclarer le repot distant pour télécharger les artefacts externes ou internes.
* ```<pluginRepository>``` : element pour déclarer le repot distant pour télécharger les plugins externes ou internes.
* ```<distributionManagement>``` : element pour déclarer le repot distant pour déployer les artefacts du projet ( snapshoot, release et site web.

:information_source: https://search.maven.org/ le référentiel maven.
Tous ces éléments peuvent etre déclarer au niveau projet ou niveau profile.

Pour se connecter au référentiel distant, maven peut utiliser les protocoles ( HTTP par défaut/HTTPS, SCP, FTP, WebDAV).
